#!/bin/sh
MAINCLASS="com.xier.sesame.attence.web.AppStarter"
JAVA_OPTS="-Dfile.encoding=UTF-8 -Dsun.jnu.encoding=UTF-8 -server -Xms512m -Xmx512m -Xmn256m -XX:+HeapDumpOnOutOfMemoryError -XX:HeapDumpPath=../logs/heap_dump.bin -XX:PermSize=64m -XX:MaxPermSize=128m -XX:+UseParallelOldGC -XX:MaxTenuringThreshold=13 -Xdebug -Xrunjdwp:transport=dt_socket,address=8666,server=y,suspend=n"
